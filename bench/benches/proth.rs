...
const N_SMALL: u8 = 8;
const K_SMALL32: u32 = 1;
const K_SMALL64: u64 = 1;

const MOD32_SMALL: u32 = K_SMALL32 * (1u32 << N_SMALL) + 1;
const MOD64_SMALL: u64 = K_SMALL64 * (1u64 << N_SMALL) + 1;

// N=31, K=1 → m=2^31+1 — near u32 width
const N_WIDE32: u8 = 31;
const K_WIDE: u32 = 1;

const MOD32_WIDE: u32 = K_WIDE * (1u32 << N_WIDE32) + 1;

// N=63, K=1 → m=2^63+1 — near u64 width
const N_WIDE64: u8 = 63;
const K_WIDE64: u64 = 1;

const MOD64_WIDE: u64 = K_WIDE64 * (1u64 << N_WIDE64) + 1;
...
pub fn bench_transform(c: &mut Criterion) {
    ...
    let reducer32 = FixedProth32::<N_SMALL, K_SMALL32>::new(&MOD32_SMALL);
    let reducer64 = FixedProth64::<N_SMALL, K_SMALL64>::new(&MOD64_SMALL);
...
pub fn bench_mul_near_width_u32(c: &mut Criterion) {
    ...
    let reducer32 = FixedProth32::<N_WIDE32, K_WIDE>::new(&MOD32_WIDE);
    let lhs32: Vec<u32> = lhs.iter().map(|&v| reducer32.transform(v)).collect();
    let rhs32: Vec<u32> = rhs.iter().map(|&v| reducer32.transform(v)).collect();
...
pub fn bench_mul_near_width_u64(c: &mut Criterion) {
    ...
    let reducer64 = FixedProth64::<N_WIDE64, K_WIDE64>::new(&MOD64_WIDE);
