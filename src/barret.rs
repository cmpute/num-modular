...
impl PreMulInv1by1<u64> {
    ...
    #[inline]
    pub const fn new_raw(divisor: u64, m: u64, shift: u32) -> Self {
        Self { divisor, m, shift }
    }
    ...
    pub const fn new(divisor: u64) -> Self {
        ...
        // n = ceil(log2(divisor))
        let n = u64::BITS - (divisor - 1).leading_zeros();
...
impl Normalized2by1Divider<u64> {
    ...
    pub const fn invert_word(divisor: u64) -> u64 {
        use crate::word::u64::*;
        let (m, _hi) = split(u128::MAX / extend(divisor));
        debug_assert!(_hi == 1);
        m
    }
    ...
    pub const fn div_rem_2by1(&self, a: u128) -> (u64, u64) {
        ...
        let (q0, q1) = split(extend(self.m) * extend(a_hi) + a);
...
impl Normalized3by2Divider<u64, u128> {
    ...
    pub const fn invert_double_word(divisor: u128) -> u64 {
        ...
        let mut v = Normalized2by1Divider::invert_word(d1);
    ...
    pub const fn div_rem_3by2(&self, a_lo: u64, a_hi: u128) -> (u64, u128) {
        ...
        let (q0, q1) = split(extend(self.m) * extend(a2) + a_hi);
        let r1 = a1.wrapping_sub(q1.wrapping_mul(d1));
        let t = extend(d0) * extend(q1);
...
mod tests {
    ...
    fn test_mul_inv_1by1() {
        ...
        for _ in 0..400000 {
            ...
            let fast_div = PreMulInv1by1::new(d);
            let n = rng.gen();
